...
async def welcome(call: types.CallbackQuery):
    order_history_button = InlineKeyboardButton(text="Показать предыдущие 5 заказов", callback_data="history_requested")
...
async def make_order(message: types.Message):
    ...
    # DAY SLOTS ARE GETTING PULLED FROM DB HERE:
...
async def make_order(message: types.Message):
    ...
    global specialist_index
    specialist_index = specialist_finder[premise_name][specialist_name]
    # TIME SLOTS ARE GETTING PULLED FROM DB HERE:
    ...
    print(times_list)
    global times_test_print
    times_test_print = times_list
